...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QTabWidget, QSplitter, QTextEdit, QComboBox
)
...
class VcfComparator:
    ...
    def compare_files(self, file1_contacts, file2_contacts, match_method="Name + Phone"):
        """Compare two lists of contacts and return differences"""
        self.file1_contacts = file1_contacts
        self.file2_contacts = file2_contacts
        
        # Find contacts only in file1
        only_in_file1 = []
        for contact in file1_contacts:
            if not self.find_contact_in_list(contact, file2_contacts, match_method):
                only_in_file1.append(contact)
        
        # Find contacts only in file2
        only_in_file2 = []
        for contact in file2_contacts:
            if not self.find_contact_in_list(contact, file1_contacts, match_method):
                only_in_file2.append(contact)
        
        # Find common contacts
        common_contacts = []
        for contact in file1_contacts:
            match = self.find_contact_in_list(contact, file2_contacts, match_method)
            if match:
...
class ComparisonWindow(QMainWindow):
    ...
    def compare_files(self):
        try:
            ...
            self.comparison_results = self.comparator.compare_files(file1_contacts, file2_contacts, match_method)
...
{len(results['common'])} contacts are present in both files
        ...
        # Update File 1 only tree
        self.file1_tree.clear()
        for index, contact in enumerate(results['only_in_file1'], start=1):
            ...
            self.file1_tree.addTopLevelItem(item)
        
        # Update File 2 only tree
        self.file2_tree.clear()
        for index, contact in enumerate(results['only_in_file2'], start=1):
            item = QTreeWidgetItem([
                str(index),
                contact.name,
                contact.phone or 'No Phone',
            ...
            self.file2_tree.addTopLevelItem(item)
        
        # Update common contacts tree
        self.common_tree.clear()
        for index, (contact1, contact2) in enumerate(results['common'], start=1):
            item = QTreeWidgetItem([
                str(index),
                contact1.name,
            ...
            self.common_tree.addTopLevelItem(item)
        
        # Update tab titles with counts
        self.results_tab.setTabText(1, f"Only in File 1 ({len(results['only_in_file1'])})")
        self.results_tab.setTabText(2, f"Only in File 2 ({len(results['only_in_file2'])})")
        self.results_tab.setTabText(3, f"Common Contacts ({len(results['common'])})")
    ...
    def export_contacts(self, contact_type):
        ...
        if contact_type == 'file1':
            contacts_to_export = self.comparison_results['only_in_file1']
            default_filename = "only_in_file1.vcf"
        elif contact_type == 'file2':
            contacts_to_export = self.comparison_results['only_in_file2']
            default_filename = "only_in_file2.vcf"
        elif contact_type == 'common':
            # For common contacts, export from file1
            contacts_to_export = [pair[0] for pair in self.comparison_results['common']]
            default_filename = "common_contacts.vcf"
        ...
        try:
            ...
            QMessageBox.information(self, "Export Success", f"Exported {len(contacts_to_export)} contacts to {file_path}")
        except Exception as e:
